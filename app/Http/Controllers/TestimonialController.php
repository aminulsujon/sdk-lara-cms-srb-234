...
class TestimonialController extends Controller
{
    ...
    {
        $contents = Content::where('content_type' , 'testimonial' )->with('upload','Tag','Comment')->orderBy('id','desc')->paginate();
        // dd($contents);
    ...
    {
        $contents = Content::where('content_type' , 'testimonial' )->with('upload')->orderBy('id','desc')->paginate();
        // $contents = Content::where('content_type' , 'work' )->with('upload','Tag','Comment')->orderBy('id','desc')->paginate();
        // dd($contents);
        return view('admin.testimonial.index',compact('contents'));
    }
    ...
    public function create()    
    {
        ...
        // dd($tags_category );
        return view('admin.testimonial.create',compact('tags','tags_category'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $content = new Content;
        $content->content_type = 'testimonial';
        ...
        if(!empty($request->details)){
            ...
            $cleanHtml = preg_replace('/\s*class\s*=\s*["\']?ql-editor["\']?/i', '', $cleanHtml);
            $cleanHtml = preg_replace('/\s*contenteditable\s*=\s*["\']?false["\']?/i', '', $cleanHtml);
            $cleanHtml = $this->removeInputFields($cleanHtml);
        }
        ...
        $result = (new LandingController)->generateLanding('testimonial',$statusCode,$request->slug);
        if($result == 2){ 
            if($content->save()){
    ...
    public function show(string $id)
    {
        ...
        // dd($content);
        return view('admin.testimonial.show',compact('content','tags','tags_category'));
    }
    ...
    public function edit(string $id)
    {
        ...
        // dd($content);
        return view('admin.testimonial.edit',compact('content'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($content->save()){
            if($request->slug != $request->oldslug){
                $result = (new LandingController)->generateLanding('testimonial',$statusCode,$request->slug);
                $result_new = (new LandingController)->addNextLanding('testimonial',301,$request->oldslug,$request->slug);
            }
            else{
                $result = (new LandingController)->updateLanding('testimonial',$statusCode,$request->slug);
            }
